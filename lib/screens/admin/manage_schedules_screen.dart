                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final subSchedules = _schedules
                                .where((s) => s['subject']?['maMonHoc'] == sub.maMonHoc)
                                .toList();
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                leading: CircleAvatar(
                                  backgroundColor: (scheduleCount > 0 ? AppTheme.primaryBlue : Colors.grey).withOpacity(0.1),
                                  child: Icon(
                                    Icons.book,
                                    color: scheduleCount > 0 ? AppTheme.primaryBlue : Colors.grey,
                                  ),
                                ),
                                title: Text(
                                  sub.tenMonHoc,
                                ...
                                trailing: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: (scheduleCount > 0 ? Colors.purple : Colors.grey).withOpacity(0.1),
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                  child: Text(
                                    scheduleCount > 0 ? '$scheduleCount Lịch học' : 'Chưa xếp lịch',
                                    style: TextStyle(
                                      color: scheduleCount > 0 ? Colors.purple : Colors.grey,
                                ...
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => SubjectSchedulesDetailScreen(
                                        subject: sub,
...
class SubjectSchedulesDetailScreen extends StatefulWidget {
  ...
  @override
  State<SubjectSchedulesDetailScreen> createState() => _SubjectSchedulesDetailScreenState();
}

class _SubjectSchedulesDetailScreenState extends State<SubjectSchedulesDetailScreen> {
      ...
      if (mounted) {
        setState(() {
          _subjectSchedules = allSchedules
              .where((s) => s['subject']?['maMonHoc'] == widget.subject.maMonHoc)
              .toList();
          _isLoading = false;
        });
      }
  ...
  void _showScheduleDialog({dynamic schedule, String? preselectedSubjectCode}) {
    ...
    // Find initial values
    String? selectedSubjectCode = preselectedSubjectCode ?? schedule?['subject']?['maMonHoc'];
    if (selectedSubjectCode == null && widget.subjects.isNotEmpty) {
      selectedSubjectCode = widget.subjects.first.maMonHoc;
    }
    
    int? selectedClassroomId = schedule?['classroom']?['id'];
    if (selectedClassroomId == null && widget.classrooms.isNotEmpty) {
      selectedClassroomId = widget.classrooms.first['id'];
    }

    String? selectedLecturerId = schedule?['lecturer']?['maGV'];
    if (selectedLecturerId == null && widget.lecturers.isNotEmpty) {
      selectedLecturerId = widget.lecturers.first['maGV'];
    }

    String? selectedDay = schedule?['thuTrongTuan'] ?? 'Thứ 2';
    
    final List<String> weekdays = ['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'Chủ nhật'];
    final List<String> slots = ['Ca sáng', 'Ca chiều', 'Ca tối'];

    String? selectedSlot = schedule?['caHoc'];
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              title: Text(isEditing ? 'Sửa Lịch Học' : 'Thêm Lịch Học', 
                ...
                child: Column(
                  ...
                  children: [
                    DropdownButtonFormField<String>(
                      ...
                      items: widget.subjects.map((sub) {
                        return DropdownMenuItem<String>(
                          value: sub.maMonHoc,
                          child: Text(sub.tenMonHoc, overflow: TextOverflow.ellipsis),
                        );
                      }).toList(),
                      onChanged: preselectedSubjectCode != null ? null : (val) {
                        setDialogState(() {
                    ...
                    DropdownButtonFormField<int>(
                      ...
                      items: widget.classrooms.map((cr) {
                        return DropdownMenuItem<int>(
                          value: cr['id'],
                          child: Text('${cr['tenPhong']} (${cr['toaNha'] ?? ''})', overflow: TextOverflow.ellipsis),
                        );
                      }).toList(),
                      onChanged: (val) {
                        setDialogState(() {
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: widget.lecturers.map((lec) {
                        return DropdownMenuItem<String>(
                          value: lec['maGV'],
                          child: Text('${lec['hoTen']} (${lec['maGV']})', overflow: TextOverflow.ellipsis),
                        );
                      }).toList(),
                      onChanged: (val) {
                        setDialogState(() {
                      ...
                      child: InputDecorator(
                        ...
                        child: Text(
                          startDate != null 
                            ...
                            : 'Chọn ngày bắt đầu',
                          style: TextStyle(color: startDate != null ? Colors.black87 : Colors.grey[600]),
                        ),
                      ),
                    ),
                    const SizedBox(height: 16),
                      ...
                      child: InputDecorator(
                        ...
                        child: Text(
                          endDate != null 
                            ...
                            : 'Chọn ngày kết thúc',
                          style: TextStyle(color: endDate != null ? Colors.black87 : Colors.grey[600]),
                        ),
                      ),
                    ),
                  ],
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    if (selectedSubjectCode == null || selectedClassroomId == null || selectedLecturerId == null || startDate == null || endDate == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Vui lòng điền đầy đủ thông tin bao gồm ngày bắt đầu và kết thúc!'))
                      );
                      return;
                    }
                    ...
                    final data = {
                      ...
                      'ngayBatDau': startDate!.toIso8601String().substring(0, 10),
                      'ngayKetThuc': endDate!.toIso8601String().substring(0, 10),
                    };

                    bool success;
                    if (isEditing) {
                      success = await widget.apiService.updateSchedule(schedule['id'], data);
                    } else {
          ...
          : _subjectSchedules.isEmpty
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ElevatedButton.icon(
                          ...
                          onPressed: () => _showScheduleDialog(preselectedSubjectCode: widget.subject.maMonHoc),
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final roomText = room != null ? 'Phòng: ${room['tenPhong']} (${room['toaNha']})' : 'Chưa xếp phòng';
                      ...
                      child: ExpansionTile(
                        leading: CircleAvatar(
                          ...
                          child: Icon(
                            ...
                            color: s['isConfirmed'] == true ? Colors.green : Colors.purple
                          ),
                        ),
                        title: Text('${s['thuTrongTuan']} - ${s['caHoc']}', style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Text(
                            ...
                            ListView.builder(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                return ListTile(
                                  ...
                                  title: Text('${student['hoTen']} (${student['maSV']})'),
                              ...
                              children: [
                                if (s['isConfirmed'] != true) ...[
                                  ElevatedButton.icon(
                                    ...
                                    onPressed: () async {
                                      setState(() => _isLoading = true);
                                      final ok = await widget.apiService.confirmSchedule(s['id']);
                                      if (ok) {
                                        _reloadData();
                                      } else {
                                        ...
                                        if (context.mounted) {
                                          ScaffoldMessenger.of(context).showSnackBar(
                                            const SnackBar(content: Text('Xác nhận lịch học thất bại!'))
                                          );
                                        }
                                      }
                                    },
                                ...
                                TextButton.icon(
                                  ...
                                  onPressed: () => _showScheduleDialog(schedule: s, preselectedSubjectCode: widget.subject.maMonHoc),
                                ),
                                const SizedBox(width: 8),
                                TextButton.icon(
